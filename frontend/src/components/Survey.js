...
class Survey extends Component {
  ...
  handleCreateSurveySubmit = () => {
    console.log(this.state);
    this.context.handleSurvey(this.state);
    //call api function for pulling inform server
  };
  ...
  render() {
    ...
    if (this.context.isAuth === false) {
      currentDisplay = (
        <SignUp logUser={this.logUser} changeUsername={this.changeUsername} />
      );
    } else if (
        ...
        <form onSubmit={this.handleSurveySubmit}>
          <div className='survey-background'>
            <div className='card '>
              ...
              <button
                className='btn btn-sm alert'
                type='submit'
                onClick={this.handleCreateSurveySubmit}
              >
                <img src={buttonImage} width='100vh' height='60vh' alt=''></img>
              </button>
            </div>
          </div>
        </form>
